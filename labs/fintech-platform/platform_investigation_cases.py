...
class PlatformInvestigationCaseExportPaths:
    cases_csv: Path
    html_report: Path


def open_platform_access_investigation_cases(
...
def export_platform_access_investigation_report(
    output_directory: str | Path,
    *,
    cases: tuple[AccessAnomalyInvestigationCase, ...],
) -> PlatformInvestigationCaseExportPaths:
    output_path = Path(output_directory)
    output_path.mkdir(parents=True, exist_ok=True)

    cases_csv = output_path / "platform_access_investigation_cases.csv"
    html_report = output_path / "platform_access_investigation_report.html"
    ...
    return PlatformInvestigationCaseExportPaths(
        cases_csv=cases_csv,
...
def _render_html_report(*, cases: tuple[AccessAnomalyInvestigationCase, ...]) -> str:
    ...
    summary_section = (
        ...
        if cases
        else "<p>No platform access investigation cases were included in this report.</p>"
    )
    cases_section = (
        _cases_to_html(cases)
        if cases
        else "<p>No platform access investigation cases were found.</p>"
    )
...
<head>
  ...
  <title>FinTech Platform Access Investigation Report</title>
  <style>
    body {{
      color: #1f2937;
...
<body>
  <h1>FinTech Platform Access Investigation Report</h1>
...
def _cases_to_html(cases: tuple[AccessAnomalyInvestigationCase, ...]) -> str:
    return _table(_case_headers(), [_case_row(investigation_case) for investigation_case in cases])
