...
from compliance_investigation_cases import AccessAnomalyInvestigationCase


@dataclass(frozen=True)
class InvestigationCaseExportPaths:
    cases_csv: Path
    html_report: Path


def export_investigation_case_report(
    ...
    *,
    ...
    cases_csv = output_path / "access_investigation_cases.csv"
    html_report = output_path / "access_investigation_report.html"
    ...
    return InvestigationCaseExportPaths(
        cases_csv=cases_csv,
...
def _render_html_report(*, cases: tuple[AccessAnomalyInvestigationCase, ...]) -> str:
    ...
    summary_section = (
        ...
        if cases
        else "<p>No investigation cases were included in this report.</p>"
    )
    cases_section = (
        _cases_to_html(cases)
        if cases
        else "<p>No access anomaly investigation cases were found.</p>"
    )
...
<head>
  ...
  <title>Access Investigation Case Report</title>
  <style>
    body {{
      color: #1f2937;
...
<body>
  <h1>Access Investigation Case Report</h1>
