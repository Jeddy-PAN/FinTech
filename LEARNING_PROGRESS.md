...
- 当前阶段：阶段 40 后已完成前端 Console / Manual 体验改造、Playwright 小型浏览器回归、结构命名复盘、FinTech 知识地图复盘、本地验证脚本整理、Mermaid 图集初版、payment provider boundary / webhook endpoint 第一版、provider intent link、provider settlement CSV demo 接入、webhook timestamp replay window 教学规则、provider webhook evidence item，以及 core banking basics / SQLite persistence / repeated hold consumption protection / statement CSV/HTML export / audit events / optimistic version 第一版；当前后续可扩展 statement activity filters，进入信贷生命周期，或先查证真实 provider docs 后设计 adapter。
...
| 2026-06-18 | 新增 payment provider boundary 第一版 | 新增 `platform_payment_provider.py` 和 `test_platform_payment_provider.py`，实现教学版 `ProviderPaymentIntent`、webhook payload 构造、HMAC-SHA256 签名/验签、webhook event 解析、event_id 幂等去重、provider 状态到内部状态映射，以及 settlement CSV 到 `ProviderSettlementRow` 的解析；新增 `docs/55-payment-provider-boundary.md`，并更新知识地图、图集和 docs 入口。本阶段不接真实 provider API、不新增真实 HTTP webhook endpoint、不改现有主支付流程；如后续引用真实 API 或签名规则，必须查证官方文档。验证：先确认新测试因模块缺失失败，随后定向测试 6 个通过。 |
| 2026-06-18 | 新增 provider webhook HTTP endpoint | `platform_api_app.py` 新增 `POST /platform/provider-webhooks` 教学版 endpoint，接收 `x-provider-signature` 和 signed provider webhook payload，复用 `ProviderWebhookEventProcessor` 完成验签、解析、event_id 幂等去重和 provider/internal 状态映射，并写入 access audit granted / denied；新增 `test_platform_api_provider_webhooks.py` 覆盖签名通过、重复事件和错误签名拒绝。仍不接真实 provider API，不把 webhook 自动接入主支付流程，不做真实密钥管理或生产级 replay window。验证：provider 模块、provider webhook endpoint 和验证脚本测试 9 个通过；`verify_labs.ps1 -SkipBrowser -SkipDemo -SkipFullLabs` 快速平台验证 174 个测试通过；如引用真实 provider 签名规则，必须查证官方文档。 |
| 2026-06-18 | 接入 provider settlement CSV demo | `demo.py` 新增 `reports/provider_settlement_sample.csv` 生成步骤，并用 `parse_provider_settlement_csv()` 解析为 `ProviderSettlementRow` 后进入现有 settlement reconciliation report；新增 `test_platform_demo_provider_settlement_csv.py` 覆盖 demo CSV 文件可被 provider parser 解析，且只为 completed run 输出结算行。仍不接真实 provider 文件格式或清算网络；如引用真实 settlement file 规范，必须查证官方或专业来源。验证：demo CSV + provider parser 定向测试 7 个通过；`verify_labs.ps1 -SkipBrowser -SkipFullLabs` 通过，fintech-platform 非浏览器测试 175 个通过，demo 可运行并输出 `Rows parsed: 4`。 |
| 2026-06-18 | 新增 provider webhook timestamp replay window | `ProviderWebhookEventProcessor` 新增可配置 `timestamp_tolerance_seconds` 和 `received_at`，用于拒绝太旧或来自明显未来的 signed webhook payload；`create_app()` 默认对 `POST /platform/provider-webhooks` 使用 300 秒教学窗口，并继续记录 granted / denied access audit；新增模块和 API 测试覆盖过期事件、未来事件和 endpoint 拒绝过期 signed payload。该规则是教学版，不代表任何真实 provider 的 header 或窗口规范；真实规则必须查证官方文档。验证：provider 相关定向测试 11 个通过；`verify_labs.ps1 -SkipBrowser -SkipFullLabs` 通过，fintech-platform 非浏览器测试 178 个通过，demo 可运行。 |
| 2026-06-18 | 新增 provider webhook evidence item | `platform_evidence_package.py` 新增 provider webhook evidence extractor，会把 `process_platform_provider_webhook` / `fintech_platform_provider_webhooks` access audit event 转成 `provider_webhook_event`，覆盖 granted、duplicate 和 denied provider webhook 处理结果；demo 新增一条成功 signed webhook 和一条过期 signed webhook 样例，并把 API access events 合并进 evidence package。该证据项仍是教学版，不代表真实法律证据链或监管报送。验证：evidence + provider 定向测试 17 个通过；`verify_labs.ps1 -SkipBrowser -SkipFullLabs` 通过，fintech-platform 非浏览器测试 179 个通过，demo 可运行并输出 provider webhook evidence sample。 |
| 2026-06-18 | 新增 provider intent link | `platform_payment_provider.py` 新增 `provider_intent_id_for_run()`、`build_provider_payment_intent()`、`build_provider_payment_intents()` 和 `export_provider_payment_intents_csv()`，用教学版 deterministic `provider_intent_id = {provider}_intent_{run_id}` 关联外部 provider intent、内部 `run_id` 和 `payment_order_id`；demo 会导出 `reports/provider_payment_intents.csv`，webhook payload 和 settlement row 也使用同一套 provider intent id。仍不接真实 provider API，不代表真实 provider 的 object id 格式。验证：provider intent/link 定向测试 13 个通过；`verify_labs.ps1 -SkipBrowser -SkipFullLabs` 通过，fintech-platform 非浏览器测试 183 个通过，demo 可运行并输出 `provider_payment_intents.csv` 和 `Intent links: 5`。 |
| 2026-06-18 | 新增 core banking basics 第一版 | 新增 `labs/core-banking-basics/core_banking.py`、`test_core_banking.py`、`README.md` 和 `docs/56-core-banking-basics.md`，实现教学版 account product、bank account、ledger balance、available balance、account hold、release/capture hold、deposit/withdraw、daily interest accrual、monthly statement、account status 和 posting 幂等；更新根 README、docs 入口和知识地图。该实验不代表真实银行产品、真实计息、费用、税务、披露或监管规则；真实规则需要查证官方或专业来源。验证：core banking 定向测试 12 个通过，非写入式语法检查通过；标准验证脚本提权运行通过，`labs/fintech-platform` 非浏览器测试 183 个通过，全量 `labs` 非浏览器测试 440 个通过。下一步可补 SQLite 持久化、事务边界和 statement 查询，或进入信贷生命周期。 |
| 2026-06-18 | 新增 core banking SQLite persistence 第一版 | 新增 `labs/core-banking-basics/sqlite_core_banking.py` 和 `test_sqlite_core_banking.py`，实现教学版 `SQLiteCoreBankingService`，把 products、accounts、postings 和 holds 落到 SQLite；支持重开数据库后查询 balance、statement、hold 状态、interest accrual 和 idempotency 记录；测试覆盖 hold capture 持久化、available balance 校验、幂等键跨重开重放/冲突、statement 数据库查询和小利率精度保存。仍不代表生产级 core banking 数据库、并发控制、迁移、灾备或真实银行产品规则。验证：非写入式语法检查通过；core banking 定向测试 21 个通过；标准验证脚本提权运行通过，`labs/fintech-platform` 非浏览器测试 183 个通过，全量 `labs` 非浏览器测试 449 个通过。下一步可补并发测试、statement CSV 导出、账户 audit events，或进入 loan lifecycle。 |
| 2026-06-18 | 新增 core banking repeated hold consumption protection | `SQLiteCoreBankingService.release_hold()` 和 `capture_hold()` 现在检查 `WHERE status = 'active'` 条件更新的 `rowcount`，如果 active hold 已被消费会抛错并回滚，避免重复 capture 留下第二条 debit posting；新增测试覆盖同一连接重复 capture、两个 SQLite 连接重复 capture 同一个 hold，以及 release 后再 capture 不产生 posting。该保护仍是教学版状态条件更新，不代表生产级锁、lease、重试策略或分布式并发控制。验证：非写入式语法检查通过；core banking 定向测试 24 个通过；标准验证脚本提权运行通过，`labs/fintech-platform` 非浏览器测试 183 个通过，全量 `labs` 非浏览器测试 452 个通过。下一步可补 statement CSV 导出、账户 audit events，或进入 loan lifecycle。 |
| 2026-06-18 | 新增 core banking statement CSV export | 新增 `core_banking_statement_export.py` 和 `test_core_banking_statement_export.py`，把 `MonthlyStatement` 导出为 summary CSV 和 postings CSV；summary 包含 account_id、period、opening/closing balance、credits/debits、interest 和 posting_count；postings 包含 posting_id、时间、方向、类型、金额、signed_amount、描述、幂等键和 request fingerprint；内存版和 SQLite 版 statement 都可复用同一导出函数。该导出仍是教学版 activity report，不代表正式银行对账单、客户通知、下载权限或电子签名。验证：非写入式语法检查通过；core banking 定向测试 28 个通过；标准验证脚本提权运行通过，`labs/fintech-platform` 非浏览器测试 183 个通过，全量 `labs` 非浏览器测试 456 个通过。下一步可补账户 audit events，或进入 loan lifecycle。 |
| 2026-06-18 | 新增 core banking audit events | 新增 `core_banking_audit.py` 和 `test_core_banking_audit.py`；内存版 `CoreBankingService` 与 SQLite 版 `SQLiteCoreBankingService` 会记录 `account.opened`、`account.status_changed`、`posting.created`、`hold.placed`、`hold.released`、`hold.captured` 和 `interest.accrued` 教学版事件；SQLite 新增 `core_banking_audit_events` 表，重开数据库后仍可查询事件；`export_monthly_statement_csv()` 可选写入 `statement.exported` 事件。该事件流仍是教学版可追溯记录，不代表真实法律证据链、监管留存、电子签名或正式审计报告。验证：core banking 定向测试 32 个通过；标准验证脚本提权运行通过，`labs/fintech-platform` 非浏览器测试 183 个通过，全量 `labs` 非浏览器测试 460 个通过。下一步可补 statement HTML report、更严格并发控制，或进入 loan lifecycle。 |
| 2026-06-18 | 新增 core banking statement HTML report | `core_banking_statement_export.py` 新增 `export_monthly_statement_html()`，把 `MonthlyStatement` 导出为静态 HTML activity report，包含 summary metrics、posting details、signed amount、HTML 转义和教学边界提示；导出时可选记录 `statement.html_exported` audit event；新增测试覆盖 HTML 内容、转义和 audit event。该 HTML 仍是教学版活动报告，不代表正式银行对账单、客户通知、电子签名、下载权限或监管披露。验证：core banking 定向测试 34 个通过；标准验证脚本提权运行通过，`labs/fintech-platform` 非浏览器测试 183 个通过，全量 `labs` 非浏览器测试 462 个通过。下一步可补更严格并发控制、statement activity filters，或进入 loan lifecycle。 |
| 2026-06-18 | 新增 core banking optimistic version | `SQLiteCoreBankingService` 的 account 表新增 `version` 字段和 `account_version_snapshot()`；`set_account_status(..., expected_version=...)` 支持按旧版本条件更新账户状态，成功后 version + 1，并在 `account.status_changed` audit event payload 中记录 previous_version / new_version；新增测试覆盖账户版本递增，以及两个 SQLite 连接不能基于同一个旧 account version 同时更新状态。该机制仍是教学版 optimistic concurrency control，不代表生产级锁、隔离级别调优、lease、retry policy、死锁处理或跨服务一致性。验证：core banking 定向测试 36 个通过；标准验证脚本提权运行通过，`labs/fintech-platform` 非浏览器测试 183 个通过，全量 `labs` 非浏览器测试 464 个通过。下一步可补 statement activity filters，或进入 loan lifecycle。 |
